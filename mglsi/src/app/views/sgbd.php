				     	...
				     	<input type="hidden" id="edit_id" name="edit_id" value="0" />
                            ...
                            <div class="form-inner">
                                <div class="control-group input-prepend">
                                    ...
                                    <div class="controls">
                                    <span class="add-on" rel="tooltip" title="Ajout des libellé de sgbd" data-placement="top"><i class="icon-edit"></i></span>
                                        <input id="sgbd" name="libelle_sgbd" class="span4" type="Text" autocomplete="false">
                                    </div>
                                </div>
                            ...
                            <div class="form-inner">
                                <div class="control-group input-prepend">
                                    <label class="control-label">Libelle modele donnée  <span class="required">*</span></label>
                                    <div class="controls">
                                    <span class="add-on" rel="tooltip" title="Ajout des libellé de modele_données" data-placement="top"><i class="icon-edit"></i></span>
                                        <select id="modele" class="span4 selectpicker show-tick required" name="libelle_modele" data-live-search="true" required >
												<option value="">-- select --</option>
                                                <?php foreach($modeles as $modele):
                                                ?>
                                                <option value="<?= $modele['ID_MODELE'] ?>"><?= $modele['LIBELLE_MODELE']?></option>
                                                <?php endforeach;?>
											</select>
                                    </div>
                                </div>
...
$(document).ready(function(e){
    $(document).on('click', '.btn-submit', function(ev){
			...
			var btn_button = $(this);
				...
				$.post('index.php?url=sgbd/create', data, function(data,status){
        ...
        $(document).on('click', '.btn-edit', function(ev){
			...
			$.ajax({
			  cache: false,
			  url: 'index.php?url=sgbd/show', // url where to submit the request
			  ...
			  success : function(result) {
				...
				$("#edit_id").val(result['ID_SGBD']);
				$("#sgbd").val(result['LIBELLE_SGBD']);
                $("#modele").val(result['ID_MODELE']).change();
        ...
        $(document).on('click', '.btn-confirm-delete', function(ev){
			...
			$.post('index.php?url=sgbd/delete', { form_name: "del_user", tbl_id: tbl_id }, function(data,status){
				console.log(data);
				if(data == "1"){
...
<div class="span8 ">
                        <div class="box pattern pattern-sandstone ">
                            <div class="box-header">
								...
								<span class="pull-right"><a href="/trig_sgbd" class ="btn-info"> voir modification</a></span>
                            </div>
                            <div class="box-content box-table ">
                                <table id="sample-table" class="table table-hover table-bordered tablesorter">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td><?= $data['ID_SGBD'] ?></td>
                                            <td><?= $data['LIBELLE_SGBD'] ?></td>
                                            <td><?= $data['LIBELLE_MODELE'] ?></td>
                                            <td class="td-actions">
                                                <button type="button" class="btn btn-small btn-info btn-edit" id="<?= $data['ID_SGBD']  ?>" title="Delete details" >
                                                <i class="btn-icon-only icon-ok"></i>
													</button>
                                                <button type="button" class="btn btn-small btn-danger btn-delete" id="<?= $data['ID_SGBD']  ?>"  title="Delete details" data-toggle="modal" data-target="#confirmModal" >
