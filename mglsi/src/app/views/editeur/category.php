...
$(document).ready(function(e){
	 $(document).on('click', '.btn-submit', function(ev){
			...
			var btn_button = $(this);
				...
				$.post('/editeur/categorie/create', data, function(data,status){
		 ...
		 $(document).on('click', '.btn-edit', function(ev){
			...
			$.ajax({
			  cache: false,
			  url: '/editeur/categorie/show', // url where to submit the request
		 ...
		 $(document).on('click', '.btn-confirm-delete', function(ev){
			...
			$.post('/editeur/categorie/delete', { form_name: "del_user", tbl_id: tbl_id }, function(data,status){
				console.log(data);
				if(data == "1"){
