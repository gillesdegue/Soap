...
$(document).ready(function(e){
	 $(document).on('click', '.btn-submit', function(ev){
			...
			var btn_button = $(this);
				...
				$.post('/editeur/article/create', data, function(data,status){
		 ...
		 $(document).on('click', '.btn-edit', function(ev){
			...
			$.ajax({
			  cache: false,
			  url: '/editeur/article/show', // url where to submit the request
		 ...
		 $(document).on('click', '.btn-confirm-delete', function(ev){
			...
			$.post('/editeur/article/delete', { form_name: "del_user", tbl_id: tbl_id }, function(data,status){
				console.log(data);
				if(data == "1"){
