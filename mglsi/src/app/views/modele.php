                            ...
                            <div class="form-inner">
                                <div class="control-group input-prepend">
                                    ...
                                    <span class="add-on" rel="tooltip" title="Ajout des libellé de modele_données" data-placement="top"><i class="icon-edit"></i></span>
                                        <input id="libelle" name="libelle_modele" class="span4" type="Text" value="" autocomplete="false">
                                    </div>
                                </div>
...
$(document).ready(function(e){
    $(document).on('click', '.btn-submit', function(ev){
			...
			var btn_button = $(this);
				...
				$.post('index.php?url=modele/create', data, function(data,status){
        ...
        $(document).on('click', '.btn-edit', function(ev){
			...
			$.ajax({
			  cache: false,
			  url: 'index.php?url=modele/show', // url where to submit the request
			  ...
			  success : function(result) {
				...
				$("#edit_id").val(result['ID_MODELE']);
				$("#libelle").val(result['LIBELLE_MODELE']);
        ...
        $(document).on('click', '.btn-confirm-delete', function(ev){
			...
			$.post('index.php?url=modele/delete', { form_name: "del_user", tbl_id: tbl_id }, function(data,status){
				console.log(data);
				if(data == "1"){
...
<div class="span8 ">
                        <div class="box pattern pattern-sandstone ">
                            <div class="box-header">
								...
								<span class="pull-right"><a href="/trig_modele" class ="btn-info"> voir modification</a></span>
                            </div>
                                ...
                                <table id="sample-table" class="table table-hover table-bordered tablesorter">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><?= $modele['LIBELLE_MODELE'] ?></td>
                                            <td class="td-actions">
                                                <button type="button" class="btn btn-small btn-info btn-edit" id="<?= $modele['ID_MODELE']  ?>" title="Delete details" >
                                                <i class="btn-icon-only icon-ok"></i>
													</button>
                                                <button type="button" class="btn btn-small btn-danger btn-delete" id="<?= $modele['ID_MODELE']  ?>"  title="Delete details" data-toggle="modal" data-target="#confirmModal" >
